...
public class Relatorio {
    private ArvoreRBRelatorio[] estado;

    public Relatorio(EstruturaRBT estruturaDeDados, ArvoreRBRelatorio[] estados, int faixaetaria1, int faixaetaria2) {
        this.estado = estados;

        percorrerRBTInOrder(estruturaDeDados.getRaiz(), faixaetaria1, faixaetaria2);
    }

    private void percorrerRBTInOrder(NoRB no, int faixaetaria1, int faixaetaria2) {
        if (no == null) {
            return;
        }

        percorrerRBTInOrder(no.getEsquerdo(), faixaetaria1, faixaetaria2);
        ...
        if ((ane - ano) >= faixaetaria1 && (ane - ano) <= faixaetaria2) {
            ...
            estado[j].inserirRBTRapido(no.getCidadao());
        }

        percorrerRBTInOrder(no.getDireito(), faixaetaria1, faixaetaria2);
    }
